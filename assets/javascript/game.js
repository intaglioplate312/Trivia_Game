
// You'll create a trivia form with multiple choice or true/false options (your choice).
//The player will have a limited amount of time to finish the quiz. 
// The game ends when the time runs out. The page will reveal the number of questions that players answer correctly and incorrectly.


// timer function
function startTimer(duration, display) {
    var timer = duration, seconds;
    setInterval(function () {

        seconds = parseInt(timer % 60, 10);
        //ternary annotation -- not sure yet...
        seconds = seconds < 10 ? "0" + seconds : seconds;
        display.textContent = "You have " + seconds + " to finish the game.";
        if (--timer === 0) {
            // not sure why but timer is jerky with out duration??
            timer = duration;
            //still need to eleminate NaN
            timer = clearInterval;
        }
    }, 1000);
}
//Call timer
$(".test").click(function () {
    var timer = 20;
        display = document.querySelector('#time');
    startTimer(timer, display);
   });


// at end of timer 
// populate with answers in random order with attached buttons -- wimped out did in HTML/
// if not place with ids, used class
//MDN sample to try if time allows
//$( "input[type=value].tags" ).val(function( index, value ) {
//return value.trim();
//});
var correctAnswer = 0;
// should work but too simple??
var incorrectAnswer = 6 - correctAnswer;

//Correct answer function
function calcscore(){
    var correctAnswer = 0;
    $(".calc:checked").each(function(){
        score+=parseInt($(this).val(),10);
    });
    $("input[name=sum]").val(correctAnswer)
}
$().ready(function(){
    $(".calc").change(function(){
        calcscore()
    });
});

          
// run function to add all on clicks matching true and return as "you matched"
// add all clicks matching false or none retun as "you missed"
// reset using different quesitons, if time allows

// You'll create a trivia form with multiple choice or true/false options (your choice).

//The player will have a limited amount of time to finish the quiz. 

// The game ends when the time runs out. The page will reveal the number of questions that players answer correctly and incorrectly.

//Don't let the player pick more than one answer per question.

//Don't forget to include a countdown timer.

